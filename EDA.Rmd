...
for (k in 1:8) {
  ...
  for (l in 1:40){
      traindata <- data[-l,]
      modelA <- lm(Criminals_100k ~ polym(BeerAle_100k, SchoolAttendance_10k, degree =  k), data = traindata)
...
modelF <- lm(Criminals_100k ~ polym(BeerAle_100k, SchoolAttendance_10k, degree =  4), data = data)
modelF2 <- lm(Criminals_100k ~ polym(BeerAle_100k, SchoolAttendance_10k, degree =  3), data = data)

```

We choose the degree of the polynomial based on the leave-one-out and leave-two-out cross-validations results, that we run on polynomials of degrees up to 8, where we keep the two best performing models, namely the polynomial regressions of degrees 3 and 4 and compare them to the linear model (with degree 1) using Information Criteria. We get for the linear model AIC=`{r AIC(model)}` and BIC =`{r BIC(model)}`, for the model of degree 3 AIC=`{r AIC(modelF2)}` and BIC =`{r BIC(modelF2)}` and for the model of degree 4 AIC=`{r AIC(modelF)}` and BIC =`{r BIC(modelF)}`. Using the likelihood ratio test, we reject le null hypothesis at a 5% significance level that the restricted linear model fits better the data and we keep the unrestricted model. Therefore, the polynomial model of degree 3 provides a significantly better fit to the data.
...
```{r}
stargazer(modelF2, type = "text", title = "Regression Results")
```

## Model assessment

We will first verify that our model assumptions hold :
...
```
Both the Autocorrelation function plot and the Durbin-Watson test suggest that the autocorrelation is not too strong, so we can neglect it especially that we have a small sample
...
Given that our data set is small, we can assume from the Normal QQ plot that our residuals are normally distributed
