...
Clay argues that it is only natural to make inquiries about relevant causes and encouragements while studying the crime rate in different counties. The study identifies the number of beer houses, public school attendance, and public worship attendance as factors that could influence the crime rate. Beer/Ale houses are represented as "temptations to animal pleasures", while the progress of popular education and the formation of religious habits provide a mitigating effect on crime. The study also points out that there are inconsistencies present in the way that law and justice are administered in different counties, leading to differences in how crime is perceived or recorded.

In our analysis, "Criminals per 100k" had a positive correlation(=0.46) with "Ale/Beer houses per 100k", a negative correlation(=0.23) with "Attendants at school per 10k", and a very insignificant correlation with Attendants at public worship per 2k. This is further illustrated in the regression plots and confidence intervals clearly highlighting trends between the significantly correlated features and the target variable. To investigate non-linear relations, we choose a polynomial regression model and compare it with the performance of a linear regression model. The degree of the polynomial regression model is selected using leave-one-out and leave-two-out cross-validation methods. It is found that models with degrees 3,4 both have a lower AIC than the linear regression model thus indicating a higher quality fit, finally choosing the degree 3 polynomial model using the likelihood ratio test.

The fitted model has positive coefficients for "Ale/Beer houses per 100k" features, negative coefficients for "Attendants at school per 10k" features, and opposite sign coefficients for "Attendants at public worship per 2k", which mainly indicates that the features cancel each other out and end up being less significant. These results align with our initial analysis and partially agree with the claims made in the study, with the difference in the significance of "Attendants at public worship", which is found to be insignificant. On analysing the residual errors, we find that they are normally distributed (using the Q-Q plots) and almost homoscedastic (from the residual vs fitted values plot). The error terms are uncorrelated with a weak positive autocorrelation at lag 1. Thus the model adheres to the assumptions made during model selection.
